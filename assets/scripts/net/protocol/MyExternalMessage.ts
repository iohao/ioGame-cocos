export interface MyExternalMessage {
  cmdCode?: number;
...
export function encodeMyExternalMessage(message: MyExternalMessage): Uint8Array {
  let bb = popByteBuffer();
  _encodeMyExternalMessage(message, bb);
...
function _encodeMyExternalMessage(message: MyExternalMessage, bb: ByteBuffer): void {
  ...
  // optional bytes dataContent = 6;
  let $dataContent = message.dataContent;
  if ($dataContent !== undefined) {
    writeVarint32(bb, 50);
    writeVarint32(bb, $dataContent.length), writeBytes(bb, $dataContent);
  }
}

export function decodeMyExternalMessage(binary: Uint8Array): MyExternalMessage {
  return _decodeMyExternalMessage(wrapByteBuffer(binary));
}

function _decodeMyExternalMessage(bb: ByteBuffer): MyExternalMessage {
  let message: MyExternalMessage = {} as any;
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      // optional bytes dataContent = 6;
      case 6: {
        message.dataContent = readBytes(bb, readVarint32(bb));
        break;
      }
